"use strict";
/*
 * COMPLETE WORKING REAL LENDING CONTRACT - All functions working
 */
...
async function deployCompleteLendingContract() {
    logger.log("\n🚀 DEPLOYING COMPLETE WORKING REAL LENDING CONTRACT");
    logger.log("==================================================");
    ...
    try {
        ...
        // Step 6: Create contract interface using node2 for borrower actions
        logger.log("\n🔄 Creating borrower interface on node2...");
        const borrowerPrivacyGroup = lendingPrivacyGroup.using(paladinNode2);
        const borrowerLendingContract = new RealLendingContract(borrowerPrivacyGroup, contractAddress);
        // Step 7: Make a payment from the borrower (using node2)
        logger.log("\n💸 Borrower making first payment from node2...");
        const paymentReceipt = await borrowerLendingContract.sendTransaction({
        ...
        // Step 8: Get loan details to prove everything is working
        logger.log("\n📊 Retrieving final loan details...");
        ...
        const filename = `complete-real-lending-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
        ...
        logger.log(`\n🎯 THIS IS A COMPLETE WORKING LENDING CONTRACT!`);
        logger.log(`🚀 REAL EPHEMERAL EVM WITH FULL LENDING LIFECYCLE!`);
        return true;
    }
    catch (error) {
...
// Run the deployment
async function main() {
    const success = await deployCompleteLendingContract();
    process.exit(success ? 0 : 1);
}
main();
//# sourceMappingURL=complete-real-lending.js.map