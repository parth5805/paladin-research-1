...
// Helper function to wait for transaction receipt with manual polling
async function waitForReceiptManually(client: PaladinClient, txId: string, timeoutMs: number = 30000): Promise<any> {
  ...
  while (Date.now() - startTime < timeoutMs) {
    try {
      ...
      if (receipt) {
        logger.log(`Receipt found for transaction ${txId}:`, receipt);
        return receipt;
      }
    } catch (error) {
  ...
  throw new Error(`Timeout waiting for receipt for transaction ${txId} after ${timeoutMs}ms`);
}
...
async function main(): Promise<boolean> {
  ...
  // Submit the privacy group creation but don't wait for deploy immediately
  const privacyGroupPromise = penteFactory.newPrivacyGroup({
    members: [verifierNode1, verifierNode2],
    evmVersion: "shanghai",
    externalCallsEnabled: true,
  });
  
  // Get the transaction ID from the promise
  const txId = await privacyGroupPromise.getTransactionId();
  logger.log(`Privacy group creation transaction submitted: ${txId}`);
  
  // Wait for the receipt manually
  const receipt = await waitForReceiptManually(paladinNode1, txId, 30000);
  
  if (!receipt || !receipt.success) {
    logger.error("Privacy group creation failed!");
    return false;
  }
  
  // Get the deployed privacy group using the contract address from the receipt
  const memberPrivacyGroup = await privacyGroupPromise;
  logger.log(`Privacy group created successfully! ID: ${memberPrivacyGroup?.group.id}`);

  // Step 2: Deploy a smart contract within the privacy group
  logger.log("Deploying a smart contract to the privacy group...");
  
  const contractDeployPromise = memberPrivacyGroup.deploy({
    abi: storageJson.abi,
    bytecode: storageJson.bytecode,
    from: verifierNode1.lookup,
  });
  
  const deployTxId = await contractDeployPromise.getTransactionId();
  logger.log(`Contract deployment transaction submitted: ${deployTxId}`);
  
  // Wait for deployment receipt manually
  const deployReceipt = await waitForReceiptManually(paladinNode1, deployTxId, 30000);
  
  if (!deployReceipt || !deployReceipt.success) {
  ...
  const storeTransaction = privateStorageContract.sendTransaction({
    from: verifierNode1.lookup,
    function: "store",
    data: { num: valueToStore },
  });
  
  const storeTxId = await storeTransaction.getTransactionId();
  logger.log(`Store transaction submitted: ${storeTxId}`);
  
  // Wait for store receipt manually
  const storeReceipt = await waitForReceiptManually(paladinNode1, storeTxId, 30000);
