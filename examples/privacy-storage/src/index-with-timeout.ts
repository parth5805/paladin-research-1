...
async function main(): Promise<boolean> {
  ...
  // Step 1: Create a privacy group for members with longer timeout
  ...
  }).waitForDeploy(30000); // 30 second timeout instead of 5 second default
  if (!checkDeploy(memberPrivacyGroup)) return false;

  logger.log(`Privacy group created, ID: ${memberPrivacyGroup?.group.id}`);

  // Step 2: Deploy a smart contract within the privacy group with longer timeout
  ...
  }).waitForDeploy(30000); // 30 second timeout
  if (!contractAddress) {
  ...
  // Store a value in the contract with longer timeout
  ...
  }).waitForReceipt(30000); // 30 second timeout
